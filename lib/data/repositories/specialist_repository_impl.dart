...
class SpecialistRepositoryImpl implements SpecialistRepository {
  ...
  Future<List<Specialist>> searchSpecialists(SearchFilters filters) async {
    ...
    if (filters.keyword != null && filters.keyword!.isNotEmpty) {
      ...
      for (final word in words) {
        where.add(
          '(name LIKE ? OR skills LIKE ? OR tags LIKE ? OR category LIKE ?)',
        );
    ...
    if (filters.requiredSkills != null && filters.requiredSkills!.isNotEmpty) {
      ...
      final specialists = results
          .map((map) => Specialist.fromMap(map))
          .toList();
      return specialists.where((specialist) {
        return filters.requiredSkills!.every(
          (skill) => specialist.skills.any(
  ...
  Future<List<MatchingScore>> getRankedSpecialists({
    ...
    final keyword = effectiveFilters.keyword?.trim();
    final hasKeyword = keyword != null && keyword.isNotEmpty;
    final textRankedScores = <({MatchingScore score, double textRelevance})>[];
    for (final specialist in specialists) {
      ...
      final textRelevance = _calculateTextRelevance(specialist, keyword);
      if (textRelevance <= 0) {
        continue;
      }

      textRankedScores.add((score: score, textRelevance: textRelevance));
    }

    if (hasKeyword) {
      textRankedScores.sort((a, b) {
        final textCompare = b.textRelevance.compareTo(a.textRelevance);
        if (textCompare != 0) {
          return textCompare;
        }
        return b.score.totalScore.compareTo(a.score.totalScore);
      });
      return textRankedScores.map((entry) => entry.score).toList();
    }
  ...
  double _calculateTextRelevance(Specialist specialist, String keyword) {
    final terms = keyword
        .toLowerCase()
        .split(RegExp(r'\s+'))
        .map((term) => term.trim())
    ...
    final skills = specialist.skills
        .map((skill) => skill.toLowerCase())
        .toList();
    final tags = specialist.tags.map((tag) => tag.toLowerCase()).toList();
    ...
    for (final term in terms) {
      ...
      if (category.contains(term)) {
        score += category == term ? 3 : 2;
      }
      for (final skill in skills) {
        if (skill.contains(term)) {
          score += skill == term ? 5 : 3;
        }
      }
      for (final tag in tags) {
        if (tag.contains(term)) {
          score += tag == term ? 2.5 : 1.5;
        }
      }
    }
  ...
  Future<int> insertSpecialist(Specialist specialist) async {
    final db = await DatabaseHelper.database;
    return await db.insert(DatabaseHelper.tableSpecialists, specialist.toMap());
  }
  ...
  Future<Map<String, dynamic>> getStatistics() async {
    ...
    final count =
        Sqflite.firstIntValue(
          await db.rawQuery(
            'SELECT COUNT(*) FROM ${DatabaseHelper.tableSpecialists}',
          ),
        ) ??
        0;
